...
const Room = (props) => {
  ...
  const [senderss, setsenderss] = useState([]);
  ...
  function shareScreen() {
    ...
    /*  navigator.mediaDevices.getDisplayMedia({cursor:true})
      .then(screenStream=>{
        senders.current.replaceTrack(senders.current.getTracks()[0],screenStream.getVideoTracks()[0])
        userVideo.current.srcObject=screenStream.srcObject
        screenStream.getTracks()[0].onended = () =>{
          senders.current.replaceTrack(senders.current.getTracks()[0],screenStream.getTracks()[1])
          partnerVideo.current.srcObject=screenStream.srcObject
        }
      })*/
  }
    ...
    if (audioMuted == false) {
      ...
      return (
        <div>
          ...
          <video
            controls
            style={{ height: 500, width: 500, float: "left" }}
            id="uservid"
            autoPlay
            ref={userVideo}
            muted
          ...
          <button
            ...
            onClick={end_stream}
          >
            <Translate content="End_Stream" />
          </button>
          <button
            class="btn btn-danger "
            style={{ margin: "5px" }}
            onClick={pause_stream}
          >
            <Translate content="Pause_Stream" />
          </button>
          <button
            ...
            onClick={shareScreen}
          >
            <Translate content="Share_screen" />
          </button>
          <span>
            {" "}
            <Translate content="video_paused" />
          </span>
        </div>
      );
    } else {
